...
class DMPrice extends DMApiMethods{
  ...
  currentCom() {
    return Math.ceil(this.data.price * this.data.sales_percent / 100);
  }
  currentRek() {
  ...
  constructor(data: TDMPrice, preset: DMPricePreset, urlTransformer: (url: string) => string,) {
    ...
    this.e_perc = new ExtRef<number>(upToHund, this.data.perc);

    this.comCalculated = new ExtComputedRef(Math.ceil, ()=>{
      return this.normCalculated.value * this.data.sales_percent / 100;
    ...
    this.currentPayment = new ExtComputedRef(Math.ceil, ()=>{
      ...
      return this.data.marketing_seller_price - this.data.fbs_direct_flow_trans_max_amount - this.#preset.data.sum_obtain - this.#preset.data.sum_pack - this.data.marketing_seller_price * (this.#preset.data.perc_ekv + this.#preset.data.perc_mil + this.data.sales_percent + this.e_adv_perc.value) / 100;
